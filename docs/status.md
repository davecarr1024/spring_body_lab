...
Finish the Phase 2/3 boundaries rather than widen the scene: make trace and
replay artifacts explicit, add their deterministic proof, and implement the
focused [Playwright browser suite](browser-testing.md). Use the geometry library
to introduce one fixed contact only after that record is clear. See the
