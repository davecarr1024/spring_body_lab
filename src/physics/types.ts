...
export type ContactSettings = Readonly<{ tolerance: number; maxCorrection: number; restitution: number; iterations: number; cellSize: number }>;
export type WorldDefinition = Readonly<{ particles: readonly ParticleDefinition[]; springs: readonly SpringDefinition[]; fixedSegments: readonly FixedSegmentDefinition[]; gravity: Vec2; dt: number; contact: ContactSettings }>;
...
export type StepResult = Readonly<{ state: WorldState; forces: readonly SpringForceRecord[]; contacts: readonly ContactRecord[]; diagnostics: readonly Diagnostic[]; events: readonly SpringBreakEvent[]; components: readonly Component[] }>;
