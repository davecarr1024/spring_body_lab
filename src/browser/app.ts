import { advanceGame, createMultiBodyLab, createRopeSwing, createSheetLift, createWeakWallBreach, liftSheet, ramWeakWall, swingRope } from "../game/index.js";
...
const sceneName = () => game.scene === "weak-wall-breach" ? "Weak wall breach" : game.scene === "rope-swing" ? "Rope swing" : game.scene === "sheet-lift" ? "Sheet lift" : "Multi-body contact lab";
const goalStatus = () => !game.goal ? "Inspect forces and contact" : game.goal.kind === "breach" ? (game.goal.achieved ? "Breach achieved" : `Break ${game.goal.requiredSpringIds.length} weak seams`) : game.goal.kind === "reach_y" ? (game.goal.achieved ? "Lift achieved" : "Lift sheet edge to marker") : (game.goal.achieved ? "Swing achieved" : "Move rope tail past marker");
...
function sceneMarkup() {
  ...
  const label = game.scene === "weak-wall-breach" ? "Weak wall breach arena" : game.scene === "rope-swing" ? "Rope swing arena" : game.scene === "sheet-lift" ? "Sheet lift arena" : "Multiple soft bodies in a fixed contact arena";
...
function render() {
  ...
  document.querySelector("#reset")!.addEventListener("click", () => { playing = false; if (frame) cancelAnimationFrame(frame); game = game.scene === "weak-wall-breach" ? createWeakWallBreach() : game.scene === "rope-swing" ? createRopeSwing() : game.scene === "sheet-lift" ? createSheetLift() : createMultiBodyLab(); render(); });
}
