import { advanceGame, createBlockRam, createBreachRun, createMultiBodyLab, createRopeSwing, createSheetLift, createWeakWallBreach, fireBreachCharge, launchRam, liftSheet, ramWeakWall, shareSceneRecipe, swingRope } from "../game/index.js";
...
function sceneMarkup() {
  ...
  const label = game.scene === "breach-run" ? "Breach Run arena" : game.scene === "weak-wall-breach" ? "Weak wall breach arena" : game.scene === "rope-swing" ? "Rope swing arena" : game.scene === "sheet-lift" ? "Sheet lift arena" : game.scene === "block-ram" ? "Block ram arena" : "Multiple soft bodies in a fixed contact arena";
...
function render() {
  ...
  document.querySelector("#breach-run")!.addEventListener("click", () => { playing = false; if (frame) cancelAnimationFrame(frame); game = createBreachRun(); render(); });
  document.querySelector("#fire-breach")!.addEventListener("click", () => { const advanced = fireBreachCharge(game); if (!advanced.result) return; game = Object.freeze({ ...advanced.game, lastResult: advanced.result }); render(); });
  document.querySelector("#reset")!.addEventListener("click", () => { playing = false; if (frame) cancelAnimationFrame(frame); game = game.scene === "breach-run" ? createBreachRun() : game.scene === "weak-wall-breach" ? createWeakWallBreach() : game.scene === "rope-swing" ? createRopeSwing() : game.scene === "sheet-lift" ? createSheetLift() : game.scene === "block-ram" ? createBlockRam() : createMultiBodyLab(); render(); });
}
